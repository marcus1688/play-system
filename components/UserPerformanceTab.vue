  ...
  >
    ...
    <div class="bg-white rounded-lg shadow">
      ...
      <div class="p-6 max-md:p-3">
        <div class="grid grid-cols-2 gap-6 max-lg:grid-cols-1 max-md:gap-3">
          <!-- Main Wallet -->
          <div class="bg-gray-50 p-4 rounded-lg max-md:p-3">
            <div
              class="flex justify-between items-start max-md:flex-col max-md:gap-2"
            >
              <div class="max-md:w-full">
                <label
                  class="text-sm font-medium text-gray-700 max-md:text-xs"
                  >{{ $t("main_wallet_balance") }}</label
                >
                <div
                  class="text-xl font-bold mt-1 max-md:text-lg max-md:mt-0.5"
                >
                  {{ currency }} {{ userData.wallet }}
                </div>
              </div>
              <button
                @click="handleCashout"
                class="px-3 py-1 bg-indigo-600 text-white text-sm rounded lg:hover:bg-indigo-500 max-md:px-2 max-md:py-2 max-md:text-xs max-md:w-full"
              >
                {{ $t("cashout") }}
              </button>
            </div>
          ...
          <div class="bg-gray-50 p-4 rounded-lg max-md:p-3">
            ...
            >
              <div class="max-md:w-full">
                <label
                  class="text-sm font-medium text-gray-700 max-md:text-xs"
                  >{{ $t("game_wallet_balance") }}</label
                >
                <div
                  class="text-xl font-bold mt-1 max-md:text-lg max-md:mt-0.5"
                >
                  {{ currency }} {{ userGameWalletBalance || "0.00" }}
                </div>
              </div>
              <LoadingButton
                :loading="isFetchBalanceButtonLoading && !isPageLoading"
                @click="fetchUserGameWalletBalance"
                class="px-3 py-1 bg-indigo-600 text-white text-sm rounded lg:hover:bg-indigo-500 max-md:px-2 max-md:py-2 max-md:text-xs max-md:w-full"
              >
                {{ $t("fetch") }}
              </LoadingButton>
          ...
          >
            <div class="flex justify-between items-start">
              <div>
                <label
                  class="text-sm font-medium text-gray-700 max-md:text-xs"
                  >{{ $t("wallet_two_balance") }}</label
                >
                <div
                  class="text-xl font-bold mt-1 max-md:text-lg max-md:mt-0.5"
                >
                  {{ currency }} {{ userData.wallettwo || "0.00" }}
                </div>
              </div>
            </div>
          </div>
          ...
          <!-- Rebate Available -->
          <div class="bg-gray-50 p-4 rounded-lg hidden max-md:p-3">
            <div
              class="flex justify-between items-start max-md:flex-col max-md:gap-2"
            >
              <div class="max-md:w-full">
                <label
                  class="text-sm font-medium text-gray-700 max-md:text-xs"
                  >{{ $t("rebate_available") }}</label
                >
                <div
                  class="text-xl font-bold mt-1 max-md:text-lg max-md:mt-0.5"
                >
                  {{ currency }} {{ formatAmount(userData.rebate) }}
                </div>
              </div>
              <button
                @click="showUpdateRebateModal = true"
                class="px-3 py-1 bg-indigo-600 text-white text-sm rounded lg:hover:bg-indigo-500 max-md:px-2 max-md:py-2 max-md:text-xs max-md:w-full"
              >
                {{ $t("update") }}
              </button>
