...
const fetchWithdrawals = async () => {
  try {
    ...
    const response = await get(
      `user/${props.userId}/withdraw?${params.toString()}`
    );
    if (response.data.success) {
      withdrawals.value = response.data.data.map((withdraw, index) => ({
        ...withdraw,
        no: index + 1,
        time: formatDate(withdraw.createdAt),
