          ...
          <tbody class="divide-y divide-gray-200">
            ...
            >
              ...
              <td class="px-6 py-4 text-sm max-md:px-3 max-md:py-2">
                <button
                  v-if="log.imageUrl"
                  @click="openReceiptModal(log.imageUrl)"
...
const fetchDeposits = async () => {
  try {
    ...
    const response = await get(
      `user/${props.userId}/deposits?${params.toString()}`
    );
    if (response.data.success) {
      deposits.value = response.data.data.map((deposit, index) => ({
        ...deposit,
        no: index + 1,
        time: formatDate(deposit.createdAt),
