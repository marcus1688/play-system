<template>
  ...
  >
    <!-- Actions -->
    <div class="flex justify-end max-md:justify-stretch">
      <button
      ...
      <div class="overflow-x-auto">
        <table class="w-full whitespace-nowrap">
          ...
          <tbody class="divide-y divide-gray-200 text-center">
            ...
            >
              <td
                class="px-6 py-4 text-sm font-medium uppercase max-md:px-3 max-md:py-3 max-md:text-xs"
              >
                {{ bank.name }}
              </td>
              <td
...
const handleAddBank = async (bankData) => {
  try {
    const { data } = await post(
      `agent/${props.userId}/bank-accounts`,
      bankData
    );
    if (data.success) {
      await Swal.fire({
  ...
  try {
    ...
    if (willDelete.isConfirmed) {
      const { data } = await deleteRequest(
        `agent/${props.userId}/bank-accounts/${bankId}`
      );
      if (data.success) {
        await Swal.fire({
          icon: "success",
