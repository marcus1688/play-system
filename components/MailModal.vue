    ...
    >
      ...
      >
        ...
        <div class="p-6 border-b max-md:p-4">
          <div class="flex items-center gap-4 max-md:gap-3">
            ...
            <h2 class="text-xl font-semibold max-md:text-lg">
              {{ initialData?._id ? $t("edit") : $t("send_new") }}
              {{ $t("mail") }}
            </h2>
          </div>
        </div>
        ...
        >
          ...
          >
            {{ initialData?._id ? $t("save_changes") : $t("send_mail") }}
          </LoadingButton>
        </div>
      </div>
    </div></Teleport
...
const handleSubmit = async () => {
  try {
    ...
    const { data } = props.initialData?._id
      ? await put(`mails/${props.initialData._id}`, formData.value)
      : await post("mails", formData.value);
