<template>
  <div>
    ...
    <!-- Tier Info Card -->
    <div class="bg-white p-4 rounded-lg shadow mb-6 max-md:p-3 max-md:mb-4">
      <h3 class="text-sm font-medium text-gray-700 mb-3 max-md:text-xs">
        {{ $t("tier_information") }}
      </h3>
      <div class="grid grid-cols-4 gap-4 max-lg:grid-cols-2 max-md:gap-2">
        <div class="bg-gray-50 p-3 rounded-lg text-center max-md:p-2">
          <div class="text-xs text-gray-500 mb-1">Tier 1</div>
        ...
        <table class="w-full text-center whitespace-nowrap">
          ...
          <tbody class="divide-y divide-gray-200">
            ...
            >
              ...
              <td
                class="px-6 py-4 text-sm font-semibold text-gray-800 max-md:px-3 max-md:py-3 max-md:text-xs"
              >
                {{ formatAmount(record.totalDeposit) }}
              </td>
              <td class="px-6 py-4 max-md:px-3 max-md:py-3">
                <span
                  :class="[
                    ...
                    getTierClass(record.tier),
                  ]"
                >
                  {{ getTierLabel(record.tier) }}
                </span>
              </td>
              <td
...
const fetchRecords = async () => {
  try {
    ...
    if (selectedTier.value !== "all") {
      params.append("tier", selectedTier.value);
    }

    const { data } = await get(`loyalty-bonus?${params.toString()}`);
    if (data.success) {
      records.value = data.data;
    }
  } catch (error) {
    console.error("Error fetching loyalty bonus records:", error);
    await Swal.fire({
      icon: "error",
      title: $t("error"),
    ...
    result.sort((a, b) => {
      ...
      if (sortConfig.value.key === "action") {
        const getPriority = (record) => {
          if (!record.claimed && record.bonusPoints > 0) return 2;
...
const handleManualRun = async () => {
  try {
    const result = await Swal.fire({
      ...
      text: $t("loyalty_manual_run_warning"),
      icon: "warning",
    ...
    const { data } = await post("loyalty-bonus/manual-run");
...
const handleExport = async () => {
  try {
    ...
    const columns = {
      ...
      periodLabel: { header: "Period", width: 25 },
      totalDeposit: { header: "Total Deposit", width: 15 },
      tier: { header: "Tier", width: 10 },
    ...
    let filename = "loyalty_bonus_report";
