        ...
        <table class="w-full text-center whitespace-nowrap">
          ...
          <tbody class="divide-y divide-gray-200">
            ...
            >
              ...
              <td class="px-6 py-4 max-md:px-3 max-md:py-3">
                <div class="flex justify-center">
                  <img
                    ...
                    :src="bank.logo"
                    alt="Bank Logo"
                    class="h-10 w-10 object-contain max-md:h-8 max-md:w-8"
                  />
              ...
              <td class="px-6 py-4 flex justify-center max-md:px-3 max-md:py-3">
                <BaseToggleSwitch
...
const tableHeaders = [
  ...
  { key: "bankname", label: "Bank Name", labelCN: "银行名称", sortable: true },
  { key: "bankcode", label: "Bank Code", labelCN: "银行代码", sortable: true },
  { key: "remark", label: "Remark", labelCN: "备注", sortable: false },
  { key: "logo", label: "Bank Logo", labelCN: "银行标志" },
...
const { get, remove, patch, delete: del } = useApiEndpoint();
  ...
  if (searchQuery.value) {
    ...
    filtered = filtered.filter(
      (bank) =>
        ...
        (bank.remark || "").toLowerCase().includes(searchTerm)
    );
  }
...
const paginatedBanks = computed(() => {
  ...
  return filteredBanks.value.slice(start, end).map((bank, index) => ({
    ...bank,
    no: start + index + 1,
  }));
});
...
const fetchBanks = async () => {
  try {
    const { data } = await get("userbanklist");
    if (data.success) {
...
const deleteBank = async (bank) => {
  ...
  if (result.isConfirmed) {
    try {
      const { data } = await del(`deleteuserbanklist/${bank._id}`);
      if (data.success) {
        await Swal.fire({
          icon: "success",
          title: $t("success"),
...
const toggleActive = async (bank) => {
  try {
    const { data } = await patch("updateactiveuserbank", {
      id: bank._id,
