          ...
          <tbody class="divide-y divide-gray-200 text-center">
            ...
            >
              ...
              <td
                class="px-6 py-4 text-sm max-md:px-3 max-md:py-3 max-md:text-xs"
              >
                <div class="flex flex-col gap-2 items-center max-md:gap-1">
                  <div>{{ formatPhoneNumber(user.phonenumber) }}</div>
                  <select
                    class="px-2 py-1 border border-gray-300 rounded text-xs w-20 max-md:w-16 max-md:px-1 max-md:py-2"
                    :value="user.userType || ''"
                    @change="handleUserTypeChange(user, $event.target.value)"
                  >
                    <option value="">-</option>
...
const { delete: del, put } = useApiEndpoint();
...
const handleUserTypeChange = async (user, newType) => {
  try {
    const typeToSend = newType === "" ? "" : newType;
    const { data } = await put(`user/${user._id}/type`, {
      userType: typeToSend,
    });
    if (data.success) {
      const userIndex = allusers.value.findIndex((u) => u._id === user._id);
      if (userIndex !== -1) {
        allusers.value[userIndex].userType =
          typeToSend === "" ? undefined : typeToSend;
      }
      await Swal.fire({
        icon: "success",
        title: $t("success"),
